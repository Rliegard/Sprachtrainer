...
class VocabularyTrainer:
    def __init__(self, master):
        ...
        self.master.bind('<Control-Key-d>', lambda e: self.set_language_pair("Deutsch", "Englisch")) # DEUTSCH IST HIER NUR ZUGEWIESEN, WIRD ABER IN create_widgets ÜBERSPRUNGEN
    ...
    def create_widgets(self):
        ...
        for lang_char, lang_name in hotkey_map.items():
            ...
            # 🟢 Hinzufügen des Shift-Hotkeys zum Button-Text
            ...
            # 🟢 Hinzufügen des Tooltips mit Shift-Hotkey
    ...
    def check_db_and_get_translation(self, word, src_lang, trg_lang):
        ...
        if ONLINE_TRANSLATION_ENABLED:
            try:
                ...
                # Speichern des neuen Vokabels in der Datenbank
                cursor.execute("""
                    INSERT OR IGNORE INTO vocabulary (source_word, source_lang, target_lang, target_word, source) 
                    VALUES (?, ?, ?, ?, 'Online')
    ...
    def find_manual_translation(self, event=None):
        ...
        if not query_word:
            self.manual_result_label.config(text="Bitte Wort eingeben.", foreground='red')
            return
        ...
        if translation:
            ...
            if source_type == "Deaktiviert":
                error_msg = f"❌ Übersetzung nicht gefunden. Online-Übersetzung ist deaktiviert."
            elif "Fehler" in source_type:
                error_msg = f"❌ Online-Übersetzungsfehler."
            else:
